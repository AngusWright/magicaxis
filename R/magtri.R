magtri = function(chains, samples=1000, thin=1, samptype='end', grid=FALSE, do.tick=FALSE, refvals=NULL, lab=NULL, draw.sig=FALSE, ...){
  chains=as.data.frame(chains)
  chaincolnames=colnames(chains)
  Nsamp=dim(chains)[1]
  Npar=dim(chains)[2]
  if(!is.null(refvals)){
    if(length(refvals) != Npar){stop('Length of refvales must be equal to number of parameters!')}
  }
  if(Npar<=1){stop('Need 2+ parameters!')}
  
  dots=list(...)
  
  if(thin>1){
    chains=chains[seq(1,Nsamp,by=thin),,drop=FALSE]
    Nsamp=dim(chains)[1]
  }
  
  if(samples>Nsamp){samples=Nsamp}
  layout(matrix(1:Npar^2,Npar,Npar)[Npar:1,])
  meanvec={}
  sdvec={}
  if(samptype=='end'){usesamps=(Nsamp-samples+1):Nsamp}
  if(samptype=='ran'){usesamps=sample(Nsamp,samples)}
  if(samptype=='thin'){usesamps=seq(1,Nsamp,length=samples)}
  for(i in 1:Npar){
    meanvec=c(meanvec,mean(chains[usesamps,i]))
    sdvec=c(sdvec,sd(chains[usesamps,i]))
  }
  par(oma=c(4.1,4.1,1.1,1.1))
  for(i in 1:Npar){
    for(j in 1:Npar){
      par(mar=c(0,0,0,0))
      xrange=range(chains[usesamps,i])
      yrange=range(chains[usesamps,j])
      if(xrange[1]==xrange[2]){
        val=xrange[1]
        xrange[1]=val-0.05
        xrange[2]=val+0.05
      }
      if(yrange[1]==yrange[2]){
        val=yrange[1]
        yrange[1]=val-0.05
        yrange[2]=val+0.05
      }
      if(i==j){
        xtemp=chains[usesamps,i]
        if(sd(xtemp)==0){xtemp=xtemp+rnorm(samples,sd=1e-3)}
        plot(density(xtemp),axes=FALSE,main='',xlim=xrange)
        ParmOff(magaxis, dots, side=1, grid=grid, grid.col='lightgrey', labels=FALSE, do.tick=do.tick, .pass_dots=FALSE, .check=FALSE)
        abline(v=meanvec[i],lty=1,col='red')
        abline(v=meanvec[i]-sdvec[i],lty=3,col='red')
        abline(v=meanvec[i]+sdvec[i],lty=3,col='red')
        if(!is.null(refvals)){
          abline(v=refvals[i],lty=1, col='blue')
        }
        box()
        if(i==1){
          plot.window(xlim=xrange,ylim=yrange)
          if(is.null(lab)){
            ParmOff(magaxis, dots, side=1, xlab=chaincolnames[i], .pass_dots=FALSE, .check=FALSE)
          }else{
            ParmOff(magaxis, dots, side=1, xlab=lab[[i]], .pass_dots=FALSE, .check=FALSE)
          }
          if(is.null(lab)){
            ParmOff(magaxis, dots, side=2, ylab=chaincolnames[j], .pass_dots=FALSE, .check=FALSE)
          }else{
            ParmOff(magaxis, dots, side=2, ylab=lab[[j]], .pass_dots=FALSE, .check=FALSE)
          }
        }
      }else{
        if(i>j){
          plot.new()
          plot.window(xlim=xrange,ylim=yrange)
          xtemp = chains[usesamps,i]
          ytemp = chains[usesamps,j]
          if(sd(xtemp)==0){xtemp=xtemp+rnorm(samples,sd=1e-3)}
          if(sd(ytemp)==0){ytemp=ytemp+rnorm(samples,sd=1e-3)}
          dots_carry = ParmOff(magaxis, dots, side=1:2, grid=grid, grid.col='lightgrey', labels=FALSE, do.tick=do.tick, .pass_dots=FALSE, .return = 'func_args', .check=FALSE)$args_out$ignore_args
          dots_carry = ParmOff(magcon, dots_carry, x=xtemp, y=ytemp, dobar=FALSE, doim=FALSE, add=TRUE, lty=c(2,1,3), xlim=xrange, ylim=yrange, h=c(sdvec[i], sdvec[j])/5, .pass_dots=TRUE, .clash='last', .return = 'func_args', .check=FALSE)$args_out$ignore_args
          ParmOff(points.default, dots_carry, x=meanvec[i], y=meanvec[j], col='red', pch=4, cex=2, .pass_dots=TRUE, .clash = 'last', .check=FALSE)
          box()
          if(draw.sig){
            abline(v=meanvec[i],lty=1,col='red')
            abline(v=meanvec[i]-sdvec[i],lty=3,col='red')
            abline(v=meanvec[i]+sdvec[i],lty=3,col='red')
            abline(h=meanvec[j],lty=1,col='darkgreen')
            abline(h=meanvec[j]-sdvec[j],lty=3,col='darkgreen')
            abline(h=meanvec[j]+sdvec[j],lty=3,col='darkgreen')
          }
          if(!is.null(refvals)){
            abline(v=refvals[i],lty=1, col='blue')
          }
          if(j==1){
            if(is.null(lab)){
              ParmOff(magaxis, dots, side=1, xlab=chaincolnames[i], .pass_dots=FALSE, .check=FALSE)
            }else{
              ParmOff(magaxis, dots, side=1, xlab=lab[[i]], .pass_dots=FALSE, .check=FALSE)
            }
          }
        }else{
          plot.new()
          plot.window(xlim=xrange,ylim=yrange)
          dots_carry = ParmOff(magaxis, dots, side=1:2, grid=grid, grid.col='lightgrey', labels=FALSE, do.tick=do.tick, .pass_dots=FALSE, .return='func_args', .check=FALSE)$args_out$ignore_args
          dots_carry = ParmOff(points.default, dots_carry, x=chains[usesamps,i], y=chains[usesamps,j], pch='.', col='darkgrey', .pass_dots=TRUE, .check=FALSE)
          ParmOff(points.default, dots_carry, x=meanvec[i], y=meanvec[j], col='red', pch=4, cex=2, .pass_dots=TRUE, .check=FALSE)
          box()
          if(draw.sig){
            abline(v=meanvec[i],lty=1,col='red')
            abline(v=meanvec[i]-sdvec[i],lty=3,col='red')
            abline(v=meanvec[i]+sdvec[i],lty=3,col='red')
            abline(h=meanvec[j],lty=1,col='darkgreen')
            abline(h=meanvec[j]-sdvec[j],lty=3,col='darkgreen')
            abline(h=meanvec[j]+sdvec[j],lty=3,col='darkgreen')
          }
          if(i==1){
            if(is.null(lab)){
              ParmOff(magaxis, dots, side=2, ylab=chaincolnames[j], .pass_dots=FALSE, .check=FALSE)
            }else{
              ParmOff(magaxis, dots, side=2, ylab=lab[[j]], .pass_dots=FALSE, .check=FALSE)
            }
          }
        }
      }
    }
  }
  output=cbind(mean=meanvec, sd=sdvec)
  rownames(output)=chaincolnames
  return(invisible(output))
}

magcorner = magtri